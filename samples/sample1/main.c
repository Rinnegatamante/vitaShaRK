...
int main() {
	...
	shark_init(NULL); // NOTE: libshacccg.suprx will need to be placed in ur0:data
	
	// Compiling fragment shader
	uint32_t size = sizeof(fragment_shader);
	SceGxmProgram *p = shark_compile_shader(fragment_shader, &size, SHARK_FRAGMENT_SHADER);
	
	// Saving compiled GXP file on SD
	if (p) {
		saveGXP(p, size, "ux0:data/clear_f.gxp");
		shark_clear_output();
	}
	
	// Compiling vertex shader
	size = sizeof(vertex_shader);
	p = shark_compile_shader(vertex_shader, &size, SHARK_VERTEX_SHADER);
	
	// Saving compiled GXP file on SD
	if (p) {
		saveGXP(p, size, "ux0:data/rgb_v.gxp");
		shark_clear_output();
	}
